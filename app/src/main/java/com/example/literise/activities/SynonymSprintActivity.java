...
/**
 * Synonym Sprint - Endless Runner Word Game
 * Collect synonyms, avoid antonyms while running
 */
public class SynonymSprintActivity extends AppCompatActivity {
    ...
    private static final int SCROLL_SPEED = 8; // pixels per frame
    private static final int JUMP_HEIGHT = 200;
    private static final int CHARACTER_Y = 600;
    ...
    private void initializeWordDatabase() {
        ...
        wordDatabase.put("happy", new WordData(
            Arrays.asList("joyful", "glad", "cheerful", "pleased", "delighted"),
            Arrays.asList("sad", "unhappy", "gloomy", "miserable")
        ));
    ...
    private void updateGame() {
        // Move all words left (simulate forward movement)
        List<RunningWord> wordsToRemove = new ArrayList<>();

        for (RunningWord word : activeWords) {
            word.move();

            // Check if word passed character without collection
            if (!word.isCollected && word.textView.getX() < -100) {
                if (word.isSynonym) {
                    // Missed a synonym
                    combo = 0;
                    cardCombo.setVisibility(View.GONE);
            ...
            // Check collision with character (when at same height)
            if (!word.isCollected && checkCollision(word)) {
    ...
    private boolean checkCollision(RunningWord word) {
        float wordX = word.textView.getX();
        float wordY = word.textView.getY();
        float charX = characterView.getX();
        float charY = characterView.getY();

        // Simple rectangle collision
        return wordX < charX + 80 && wordX + word.textView.getWidth() > charX &&
               wordY < charY + 80 && wordY + word.textView.getHeight() > charY;
    }
    ...
    private void spawnRandomWord() {
        ...
        RunningWord runningWord = new RunningWord(word, isSynonym);
    ...
    private void jump() {
        ...
        isJumping = true;
        float originalY = characterView.getY();

        // Jump up
        characterView.animate()
            .translationYBy(-JUMP_HEIGHT)
            .setDuration(300)
            .withEndAction(() -> {
                // Fall down
                characterView.animate()
    ...
    class RunningWord {
        ...
        RunningWord(String word, boolean isSynonym) {
            this.word = word;
            ...
            // Position at right side of screen at random height
            int screenWidth = gameContainer.getWidth();
            int randomHeight = CHARACTER_Y + random.nextInt(3) * 100 - 100;
            ...
            textView.setY(randomHeight);
        }

        void move() {
            textView.setX(textView.getX() - SCROLL_SPEED);
        }
    }

    /**
     * Platform Class (unused but kept for future expansion)
     */
    class Platform {
        View view;
    }
