...
public class ModulePathView extends View {
    ...
    private void drawNode(Canvas canvas, NodeView node) {
        ...
        int size = node.isFinalAssessment() ? 200 : 140;
        ...
        switch (node.getState()) {
            case LOCKED:
                gradient = new RadialGradient(x, y - size / 4, size / 2f,
                        Color.parseColor("#9E9E9E"), Color.parseColor("#757575"),
                        Shader.TileMode.CLAMP);
                break;
            case UNLOCKED:
                gradient = new RadialGradient(x, y - size / 4, size / 2f,
                        Color.parseColor("#66BB6A"), Color.parseColor("#43A047"),
                        Shader.TileMode.CLAMP);
                break;
            case CURRENT:
                gradient = new RadialGradient(x, y - size / 4, size / 2f,
                        Color.parseColor("#FFA726"), Color.parseColor("#FB8C00"),
                        Shader.TileMode.CLAMP);
                break;
            case COMPLETED:
                gradient = new RadialGradient(x, y - size / 4, size / 2f,
                        Color.parseColor("#42A5F5"), Color.parseColor("#1E88E5"),
                        Shader.TileMode.CLAMP);
                break;
            case MASTERED:
                gradient = new RadialGradient(x, y - size / 4, size / 2f,
                        Color.parseColor("#FFD54F"), Color.parseColor("#FFA000"),
                        Shader.TileMode.CLAMP);
                break;
            default:
                gradient = new RadialGradient(x, y - size / 4, size / 2f,
    ...
    /** No-op stub — kept for API compatibility with ModuleLadderActivity. */
    public void setModuleColor(int main, int bottom) {
        // colours are baked into drawNode() per state; this method is unused
    }
